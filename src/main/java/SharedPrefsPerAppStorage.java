...
public class SharedPrefsPerAppStorage implements User.Storage {
    ...
    @Override
    @Nullable
    public Set<JSONObject> tokenDetailsForSeparatedAppsThatCanBeExchangedForTokenForCurrentApp() {
        Set<JSONObject> tokenDetailsSet = new HashSet<>();
            ...
            if (tokenDetails != null) {
                try {
                    ...
                    if (email != null) { // Need to check email so that anonymous sessions aren't included, as they can't be exchanged.
                        tokenDetailsSet.add(tokenDetails);
                    }
                } catch (JSONException e) {
                    Log.e(TAG, e.getLocalizedMessage());
